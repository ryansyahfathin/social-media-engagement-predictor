...
@st.cache_data
def load_data(path):
    return pd.read_excel(path, sheet_name='Working File')

# --- Load Dataset ---
excel_path = "social_media_engagement_data.xlsx"
if os.path.exists(excel_path):
    df = load_data(excel_path)
    st.success("✅ Data berhasil dimuat.")
else:
    st.error(f"❌ File '{excel_path}' tidak ditemukan. Silakan upload atau pastikan path benar.")
    st.stop()
